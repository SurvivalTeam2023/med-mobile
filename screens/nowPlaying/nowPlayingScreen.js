...
const songTestUri = "https://mp3-s1-zmp3.zmdcdn.me/6c15cefab4be5de004af/218878317877648665?authen=exp=1686840645~acl=/6c15cefab4be5de004af/*~hmac=a437548ce313e70ca44472a60e52f084&fs=MTY4NjY2Nzg0NTM4Nnx3ZWJWNnwwfDI3LjY0LjMwLjIzNQ";
    ...
    useEffect(() => {
        const timer = setInterval(async () => {
            if(sound && soundStatus.isSoundLoaded){
                ...
                setSoundStatus({...soundStatus, positionMillis: status.positionMillis, durationMillis: status.durationMillis, songRunningInPercentage: +((status.positionMillis/status.durationMillis)*100).toFixed(0)})
            }
        }, 1000);
        return () => {
            clearInterval(timer);
        ...
        return (
            <View style={styles.songProcessSliderWrapStyle}>
                <Slider
                    ...
                    onValueChange={(value) =>
                        handleChangeSoundTimeline(value)
                    }
                    maximumValue={100}
                    minimumValue={0}
                    style={{height: 12.0}}
    ...
    function songTimeInfo() {
        return (
            <View style={styles.songTimeInfoWrapStyle}>
                ...
                <Text style={{...Fonts.grayColor10Medium}}>{((soundStatus.durationMillis)/60000).toFixed(2) }</Text>
            </View>
        );
    }
